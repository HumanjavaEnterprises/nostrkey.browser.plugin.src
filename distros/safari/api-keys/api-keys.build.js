(() => {
  ...
  async function setSyncRespectingQuota(payload, placedEntries, setFn, nowMs) {
    const attempt = { ...payload };
    let activeKeys = placedEntries.flatMap((e) => e.keys);
    const dropOrder = [...new Set(placedEntries.map((e) => e.priority))].filter((p) => p > PRIORITY.P1_PROFILES).sort((a, b) => b - a);
    while (true) {
    ...
    try {
      ...
      try {
        ...
        const orphanKeys = Object.keys(existing).filter(
          (k) => k !== SYNC_META_KEY && !writtenKeys.includes(k)
        );
